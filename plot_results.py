...
NONE_MEAN = 'none_mean_gbps'
NONE_STDDEV = 'none_stddev_gbps'
...
def extract_means(data):
    ...
    for label in sorted(data):
        ...
        if NONE_MEAN in data[label]:
            means_none.append(data[label][NONE_MEAN])
        else:
            means_none.append(0.0)

    return (means_ecmp, means_gff, means_none)
...
def extract_stddevs(data):
    ...
    for label in sorted(data):
        ...
        if NONE_STDDEV in data[label]:
            stddevs_none.append(data[label][NONE_STDDEV])
        else:
            stddevs_none.append(0.0)

    return (stddevs_ecmp, stddevs_gff, stddevs_none)


def plot(data):
    (means_ecmp, means_gff, means_none) = extract_means(data)
    (stddevs_ecmp, stddevs_gff, stddevs_none) = extract_stddevs(data)
    ...
    for r in xrange(n_rows):
        ...
        row_none_avgs = means_none[lower:upper]
        row_none_devs = stddevs_none[lower:upper]
...
def plot(data):
    (means_ecmp, means_gff, stddevs_none) = extract_means(data)
    (stddevs_ecmp, stddevs_gff, stddevs_none) = extract_stddevs(data)
    traffics = sorted(data.keys())
